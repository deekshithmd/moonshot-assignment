...
export default function HomePage() {
    ...
    useEffect(() => {
        setToken(String(sessionStorage.getItem('auth-token')))
    }, [])

    const userData = api.user.getUser.useQuery({ token })
                                ...
                                return (
                                    <div key={item?.id} className='flex items-center gap-x-2'>
                                        <input
                                            ...
                                            onChange={e => updateInterest.mutate({ categoryId: item?.id, token: String(sessionStorage.getItem('auth-token')) })}
